...
let test_map () =
  check "Dict:map (fun x -> x + 1) {a=1,b=2,c=3,d=4}"
    (EvtDict
       [(("a"), (EvtInt 2)); (("b"), (EvtInt 3));
        (("c"), (EvtInt 4)); (("d"), (EvtInt 5))]);
  checkfail "Dict:map \"fail\" \"fail\" \"fail\""

let test_fold () =
  check "Dict:foldl (fun acc x -> x + acc) 10 {a= 1, b= 2, c= 3, d= 4}" (EvtInt 20);
  check "Dict:foldl (fun x y -> x - y) 10 {a=1, b=2, c=3}" (EvtInt 4);
  check "Dict:foldr (fun x y -> x - y) 10 {a=1, b=2, c=3}" (EvtInt (-8));
  checkfail "Dict:foldl \"fail\" \"fail\"  \"fail\" 0"
...
let test_filter () =
  check "Dict:filter (fun x -> x > 3) {a=1,b=2,c=3,d=4}"
    (EvtDict [(("d"), (EvtInt 4))])
