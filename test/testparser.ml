...
let test_dicts () =
  ...
  (Dict
   [((String "a"), (NumInt 1));
     ((String "b"), (List [(NumInt 1); (NumInt 2); (NumInt 3)]));
     ((NumInt 45), (Lambda (["x"], (Symbol "x"))))]);
...
let test_random_hell () =
  ...
  (List [(Sequence [(NumInt 20); Unit; (List [(NumInt 0); (Sequence [(List []);
  (List []); (List [(NumInt 1)]); (Letrec ("f", (Lambda (["n"], (IfThenElse ((Lt
  ((Symbol "n"), (NumInt 2))), (Symbol "n"), (Apply ((Symbol "f"), [(Sub ((Symbol
  "n"), (NumInt 1)))])) )) )), (Apply ((Symbol "f"), [(NumInt 3)])))) ]) ]) ]);
...
let test_pipeline () =
  ...
  (IfThenElse ((Lt ((Symbol "n"), (NumInt 2))), (Symbol "n"), (Plus ( (Apply
  ((Symbol "fib"), [(Sub ((Symbol "n"), (NumInt 1)))])), (Apply ((Symbol
  "fib"), [(Sub ((Symbol "n"), (NumInt 2)))])) )) )) )), (Symbol "fib"))),
