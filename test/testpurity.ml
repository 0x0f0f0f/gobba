...
let test_pur () =
  ...
  check     "let bad = fun x -> IO:print_endline x in bad" (Closure (None, "x",
  ...
  check     "fun x -> IO:print_endline x"  (Closure (None, "x",
   (Apply ((Binop (Getkey, (Symbol "IO"), (Symbol "print_endline"))),
      (Symbol "x"))),
   []));
  check     "{bad = fun x -> IO:print_endline x}:bad" (Closure (None, "x",
   (Apply ((Binop (Getkey, (Symbol "IO"), (Symbol "print_endline"))),
      (Symbol "x"))),
   [("bad",
     (Closure ((Some "bad"), "x",
